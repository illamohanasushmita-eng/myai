...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[TASK-CREATE] Checking if user profile exists for userId:",
      taskData.user_id,
    );
    ...
    if (userCheckError && userCheckError.code !== "PGRST116") {
      ...
      console.error(
        "[TASK-CREATE] Error checking user profile:",
        userCheckError,
      );
    }
    ...
    if (!userExists) {
      console.warn(
        "[TASK-CREATE] User profile not found, attempting to create it...",
      );
