...
export async function getUserSmartDevices(
  userId: string,
): Promise<SmartDevice[]> {
  try {
...
export async function getSmartDevice(
  deviceId: string,
...
export async function createSmartDevice(
  ...
  deviceData: Omit<
    SmartDevice,
    "device_id" | "user_id" | "created_at" | "updated_at"
  >,
): Promise<SmartDevice> {
  try {
    const { data, error } = await supabase
...
export async function getRecentDeviceLogs(
  deviceId: string,
  hours: number = 24,
): Promise<DeviceLog[]> {
  try {
