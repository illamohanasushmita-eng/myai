...
import { Habit, HabitLog, GrowthGoal, LearningModule } from '@/lib/types/database';
...
export async function getUserGrowthGoals(userId: string): Promise<GrowthGoal[]> {
  try {
    const { data, error } = await supabase
      .from('growth_goals')
      .select('*')
...
export async function createGrowthGoal(
  userId: string,
  goalData: Omit<GrowthGoal, 'goal_id' | 'user_id' | 'created_at' | 'updated_at'>
): Promise<GrowthGoal> {
  try {
    const { data, error } = await supabase
      .from('growth_goals')
...
export async function createLearningModule(
  userId: string,
  moduleData: Omit<LearningModule, 'module_id' | 'user_id' | 'created_at' | 'updated_at'>
