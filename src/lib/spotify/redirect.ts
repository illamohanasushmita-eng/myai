 ...
 * 2. On Desktop (Windows/Mac/Linux): Use direct URI scheme navigation
...
export async function playTrackWithAutoPlay(
  ...
  try {
    console.log(
      `🎵 [SPOTIFY REDIRECT] Initiating auto-play for: ${trackName} (ID: ${trackId})`,
    );
    ...
    console.log(
      `🎵 [SPOTIFY REDIRECT] Calling /api/spotify/play for auto-play`,
    );
    ...
    if (data.success) {
      console.log(
        `✅ [SPOTIFY REDIRECT] Auto-play successful for: ${trackName}`,
      );
      return true;
    } else {
      console.warn(
    ...
    try {
      ...
      console.error(
        "❌ [SPOTIFY REDIRECT] Fallback also failed:",
        fallbackError,
      );
    }
    return false;
