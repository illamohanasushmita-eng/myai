...
export async function routeIntent(
  ...
  try {
    ...
    if (
      ...
      console.log(
        `🎵 [INTENT ROUTER] Extracted and sanitized music query: "${musicQuery}"`,
      );
      ...
      const isSpecificQuery =
        ...
        ![
          ...
          "some music",
          "some songs",
        ].includes(musicQuery.toLowerCase());

      if (isSpecificQuery) {
        console.log(
          `🎵 [INTENT ROUTER] Specific query detected: "${musicQuery}"`,
        );

        // Try to search for the track first
        try {
          console.log(
            `🎵 [INTENT ROUTER] Searching Spotify for: "${musicQuery}"`,
          );
          ...
          if (searchResponse.ok) {
            ...
            if (searchData.tracks && searchData.tracks.length > 0) {
              ...
              console.log(
                `🎵 [INTENT ROUTER] Found track: ${trackName} (ID: ${trackId})`,
              );

              // Priority 1: Try URI scheme first (direct app opening)
              console.log(
                `🎵 [INTENT ROUTER] Attempting URI scheme redirect (Priority 1)`,
              );
              let appOpenFailed = false;
              await playInSpotifyApp(trackId, (reason: string) => {
                console.log(
                  `🎵 [INTENT ROUTER] URI scheme fallback triggered: ${reason}`,
                );
                appOpenFailed = true;
              });
              ...
              if (userId && appOpenFailed) {
                console.log(
                  `🎵 [INTENT ROUTER] Setting up auto-play fallback with userId: ${userId}`,
                );
                // Schedule auto-play attempt after 2.5 seconds (after URI scheme timeout)
                setTimeout(async () => {
                  console.log(
                    `🎵 [INTENT ROUTER] Attempting auto-play fallback (Priority 2)`,
                  );
                  const autoPlaySuccess = await playTrackWithAutoPlay(
                    trackId,
                    userId,
                    trackName,
                  );
                  if (!autoPlaySuccess) {
                    console.warn(
                      `🎵 [INTENT ROUTER] Auto-play also failed, user will see web player`,
                    );
                  }
                }, 2500);
              }
              ...
              console.log(
                `🎵 [INTENT ROUTER] No tracks found, falling back to search`,
              );
              // Fallback to search if no results
              await searchInSpotifyApp(musicQuery, (reason: string) => {
                console.log(
                  `🎵 [INTENT ROUTER] Search fallback triggered: ${reason}`,
                );
              });
            ...
            console.error(
              `🎵 [INTENT ROUTER] Search API error: ${searchResponse.status}`,
            );
        ...
        console.log(
          `🎵 [INTENT ROUTER] Generic music request, using search for: "${musicQuery || "favorite songs"}"`,
        );
    ...
    if (
      ...
      try {
        ...
        console.log(
          `📋 Voice feedback completed (${(feedbackEndTime - feedbackStartTime).toFixed(0)}ms)`,
        );
      } catch (error) {
      ...
      try {
        ...
        if (context.onNavigate) {
          ...
          console.log(
            "📋 onNavigate callback executed",
            `(${(navEndTime - navStartTime).toFixed(0)}ms)`,
          );
    ...
    if (
      ...
      return reminderText
        ...
        : "Opening add reminder page";
    }

    if (
      intent === "reminders.open" ||
      intent === "reminders.show" ||
      intent === "reminders_open" ||
      ...
      if (description && description.trim().length > 0) {
        ...
        try {
          if (context.onAddReminder) {
            console.log(
              "📌 Calling onAddReminder with description:",
              description,
              "time:",
              time,
            );
            ...
            return time
              ? `Reminder set: ${description} at ${time}`
              : `Reminder set: ${description}`;
          } else {
    ...
    if (intent === "navigate") {
      ...
      if (path) {
        try {
          ...
          if (context.onNavigate) {
            ...
            const navEndTime = performance.now();
            console.log(
              "🗺️ onNavigate callback executed",
              `(${(navEndTime - navStartTime).toFixed(0)}ms)`,
            );
          } else if (context.router) {
        ...
        try {
          ...
          speak(feedbackText, true).catch((err) =>
            console.log("🗺️ TTS error (non-critical):", err),
          );
        } catch (error) {
      ...
      console.log("🗺️ Could not determine page from:", {
        page,
        userText,
        entities,
      });
    ...
    if (intent === "general_unknown") {
      ...
      const looksLikeDescription = descriptionPatterns.some((pattern) =>
        pattern.test(lowerUserText),
      );

      if (looksLikeDescription) {
        console.log(
          "💡 Detected possible task/reminder description without trigger word",
        );
    ...
    if (
      intent === "navigation.open_page" ||
      intent === "navigation_open_page" ||
      intent === "navigate"
    ) {
...
function extractMusicQuery(
  ...
  if (entities.songName) {
    ...
    if (
      songName &&
      songName.toLowerCase() !== "song" &&
      songName.toLowerCase() !== "a song"
    ) {
      return songName;
    }
  }
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      ...
      if (
        query.toLowerCase() !== "song" &&
        query.toLowerCase() !== "a song" &&
        query.length > 0
      ) {
        return query;
      }
...
function extractPageName(
  ...
  const sortedPatterns = Object.entries(pagePatterns).sort(
