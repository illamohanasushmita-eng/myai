...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[REMINDER-CREATE] Reminder data prepared for userId:', reminderData.user_id);

    // Check if user profile exists
    console.log('[REMINDER-CREATE] Checking if user profile exists for userId:', reminderData.user_id);
    ...
    if (userCheckError && userCheckError.code !== 'PGRST116') {
      ...
      console.error('[REMINDER-CREATE] Error checking user profile:', userCheckError);
    }

    if (!userExists) {
      console.warn('[REMINDER-CREATE] User profile not found, attempting to create it...');
