...
const ReminderItem = memo(({ reminder, formatTime, isPast: isPastReminder }: {
  ...
  <div className={`flex items-center gap-4 rounded-lg backdrop-blur-sm p-3 shadow-sm ${isPastReminder ? 'bg-white/50 opacity-60' : 'bg-white/70'}`}>
    <div className={`flex size-12 shrink-0 items-center justify-center rounded-lg ${isPastReminder ? 'bg-gray-400/20 text-gray-600' : 'bg-blue-500/20 text-blue-600'}`}>
    ...
    <div>
      ...
      <p className="text-sm text-gray-600">{formatTime(reminder.reminder_time)}</p>
...
export default function RemindersPage() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log('📌 Reminders page came into focus, refetching reminders...');
        fetchReminders();
      }
    };
  ...
  const { upcomingReminders, pastReminders } = useMemo(() => {
    const upcoming = reminders.filter(r => !isPast(new Date(r.reminder_time)));
    ...
    <div className="font-display flex h-auto min-h-screen w-full flex-col bg-[#D6EAF8]">
      <div className="flex-grow pb-20">
        <header className="flex items-center bg-transparent p-4 pb-2">
          <Button asChild variant="ghost" size="icon" className="flex size-10 shrink-0 items-center justify-center rounded-full text-gray-800">
            <Link href="/dashboard">
        ...
        <main className="px-4">
          ...
          {!loading && upcomingReminders.length === 0 && pastReminders.length === 0 && (
            <div className="text-center py-8">
