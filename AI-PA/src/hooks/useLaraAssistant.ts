...
export function useLaraAssistant(
  ...
  function convertCohereToIntent(text: string, cohereData: any): Intent {
    ...
    switch (intentType) {
      ...
      case 'play_music':
        intent.musicQuery = entities.artist || entities.genre || entities.mood || 'favorite songs';
        break;
  ...
  function fallbackIntentClassification(text: string): Intent {
    ...
    if (lowerText.includes('task') && (lowerText.includes('add') || lowerText.includes('create'))) {
      return {
        intent: 'add_task',
        query: text,
    ...
    if (lowerText.includes('play') && (lowerText.includes('music') || lowerText.includes('song'))) {
      return {
        intent: 'play_music',
        query: text,
  ...
  }, [recordForFixedDuration, geminiSTT, classifyIntent, stopWakeWordListener, restartWakeWordListener, options]);
  ...
  const handlePlayMusic = useCallback(async (musicQuery: string): Promise<string> => {
    try {
  ...
  const handleAddTask = useCallback(async (taskText: string): Promise<string> => {
    try {
  ...
  const processVoiceCommand = useCallback(async (audioBlob: Blob): Promise<void> => {
    try {
      ...
      console.error('❌ [LARA] Voice command processing failed:', errorMessage);
  ...
  const executeAction = useCallback(async (intent: Intent): Promise<string> => {
    try {
